...
export function AdminDashboard({ user, onLogout }: AdminDashboardProps) {
  ...
  const [countOfBehbod, setcountOfBehbod] = useState<number>();
  ...
  const fetchDoctors = async () => {
    try {
      ...
      let totalPetions:number = 0
      let totalBebod:number = 0
      for (let d of res.data?.data.doctors || []){
        const doctorProfile = await userService.getProfileDoctor(d._id)
        totalPetions += doctorProfile.data?.data.mariz || 0
        totalBebod += doctorProfile.data?.data.behbod || 0
      }
      setcountOfBehbod(totalBebod)
      setcountOfPetions(totalPetions)
    } catch (err: any) {
  ...
  const handleAddDoctor = async (e: React.FormEvent) => {
    ...
    try {
      const user: CreateUserRequest = {
        ...
        password: doctor.password || 'Ff123456!',
        specialty: doctor.specialty || 'روان پرستاری',
      ...
      if (res.error || !res.success) {
        ...
        Swal.fire('🙂', 'همیار سلامت ساخته شد', 'success');
  ...
  const handleDeleteDoctor = async (id: string) => {
    try {
      ...
      if (res.success) {
        Swal.fire('🙂', 'همیار سلامت حذف شد', 'success');
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-6 mb-6 sm:mb-8">
          <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-800 mb-1 text-sm sm:text-base">تعداد همیاران</p>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
            <h2 className="px-2 py-3 text-right text-gray-800 ">مدیریت همیاران سلامت</h2>
            <button
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-right text-gray-800">نام همیار</th>
                  <th className="px-6 py-3 text-right text-gray-800">شماره همراه</th>
                  <th className="px-6 py-3 text-right text-gray-800">تخصص</th>
                  <th className="px-6 py-3 text-right text-gray-800">تعداد بیماران</th>
                  <th className="px-6 py-3 text-right text-gray-800">عملیات</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {filteredDoctors.map((doctor) => (
                  <tr key={doctor._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-gray-800">{doctor.fullName}</td>
                    <td className="px-6 py-4 text-gray-800">{doctor.phoneNumber}</td>
                    <td className="px-6 py-4 text-gray-800">{doctor.specialty}</td>
                    <td className="px-6 py-4 text-gray-800">{doctor.patientCount}</td>
            ...
            {filteredDoctors.map((doctor) => (
              <div key={doctor._id} className="p-4 border border-gray-200 rounded-lg">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <p className="text-gray-800">بیماران: {doctor.patientCount}</p>
                </div>
              </div>
            ))}
          </div>
